...
export default function SecretariaSucursalSelection() {
  ...
  return (
    <main className="relative pt-safe-24 pb-40 px-6 max-w-5xl mx-auto z-10 w-full">
      <div className="mb-10 flex flex-col items-center">
        ...
        <h2 className="mt-8 font-headline text-3xl font-extrabold text-white text-center leading-tight">Selección de Sucursal</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sucursales.map((suc: any, idx: number) => {
            ...
            return (
              ...
              >
                <div className={`p-3 rounded-2xl w-fit mb-6 transition-colors ${isActive ? "bg-[var(--color-primary-container)]/40" : "bg-[var(--color-primary)]/20 group-hover:bg-[var(--color-primary)]/40"}`}>
                  <span className="material-symbols-outlined text-[var(--color-primary-fixed)] text-3xl" style={isActive ? { fontVariationSettings: "'FILL' 1" } : {}}>
      ...
      <div className="mt-16 flex flex-col md:flex-row items-center justify-center gap-6">
        <Link
          ...
          className={`w-full text-center md:w-auto px-12 py-4 rounded-full bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-container)] text-white font-headline font-bold text-lg shadow-[0_10px_30px_rgba(0,105,112,0.4)] hover:shadow-[0_15px_40px_rgba(0,163,173,0.5)] active:scale-95 transition-all duration-200 ${!selectedId ? "opacity-40 pointer-events-none" : ""}`}
        >
          Continuar
        </Link>
      </div>

...
export default function SecretariaDoctorSelection() {
    ...
    <main className="relative pt-safe-24 pb-32 px-6 max-w-2xl mx-auto w-full z-10">
      ...
      <div className="mb-8">
        <div className="relative group">
          <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
            <span className="material-symbols-outlined text-cyan-400/60 group-focus-within:text-cyan-400 transition-colors">search</span>
          </div>
          <input
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((n) => <div key={n} className="h-32 bg-white/5 animate-pulse rounded-[2.5rem]" />)}
        </div>
      ) : filtered.length === 0 ? (
              ...
              >
                <div className="flex gap-5 items-start">
                  <div className="w-20 h-20 rounded-2xl bg-[var(--color-primary-container)]/20 flex items-center justify-center border border-white/10">
                    <span className="material-symbols-outlined text-4xl text-cyan-200">stethoscope</span>
                  </div>
                  <div className="flex-1 space-y-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-headline font-bold text-xl text-white">{usr?.nombre} {usr?.apellidos}</h3>
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-headline font-extrabold text-cyan-400 text-lg">${dr?.precio_consulta ?? 0} MXN</p>
                      </div>
                    </div>
                  </div>
                </div>
